 ...
 * creating another carrier shipment. All labels, including manual and sandbox
 * labels, are preserved exactly as issued.
 */
export async function POST(request: NextRequest, { params }: { params: { orderId: string } }) {
  ...
  try {
    ...
    // Reprinting must preserve the parcel weight, address, service and AWB
    // that were actually dispatched. Never regenerate from mutable order data.
    return NextResponse.json({ labelUrl: labelUrl(order.id), awb: shipment.tracking_number, regenerated: false });
  } catch {
