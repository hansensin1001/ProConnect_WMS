...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (![body.shippingAddress, body.shippingCity, body.shippingPostcode].every((value) => value == null || isSafeText(value, 500))) throw new Error("Invalid shipping details.");
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const body = await request.json();
    if (!isUuid(body?.orgId) || !isUuid(body?.orderId) || !["ALLOCATED", "SHIPPED"].includes(body?.status)) throw new Error("Invalid order transition.");
    const supabase = await requireOrderManager(body.orgId);
    ...
    const { error } = await (supabase.rpc as any)(body.status === "ALLOCATED" ? "allocate_sales_order" : "fulfill_sales_order", { p_sales_order_id: body.orderId });
    if (error) throw error;
