...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: profiles, error: profileError } = await supabase.from("user_profiles").select("user_id, user_code");
    if (profileError) throw profileError;
    const profileByUserId = new Map((profiles ?? []).map((profile) => [profile.user_id, profile.user_code]));
    const usersById = new Map((users?.users ?? []).map((user) => [user.id, { email: user.email ?? "Unknown user", disabled: user.banned_until != null, user_code: profileByUserId.get(user.id) ?? "USR?" }]));
