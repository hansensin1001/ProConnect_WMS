...
function ProConnectBrand({ footer = false }: { footer?: boolean }) {
  return <span className={`landing-brand-lockup ${footer ? "landing-brand-lockup-footer" : ""}`}>
    <span className="landing-brand-photo" aria-hidden="true" />
    <span className="landing-brand-words"><strong>ProConnect</strong><span>Express</span><small>WMS PLATFORM</small></span>
  </span>;
}
...
export default function Home() {
  return <main className="landing-page min-h-screen overflow-hidden bg-[#f7f8f8] text-ink">
    ...
    <footer className="border-t border-ink/10 bg-white"><div className="landing-shell grid gap-8 py-9 text-sm text-graphite lg:grid-cols-[1fr_auto_1fr] lg:items-center"><div><ProConnectBrand footer /></div><p className="text-center text-xs leading-6">© 2026 Proconnect Express Services (JM0993273-A). All rights reserved.</p><div className="flex flex-wrap justify-start gap-x-5 gap-y-2 lg:justify-end"><a href="https://www.proconnect-express.com/home" target="_blank" rel="noreferrer" className="font-medium text-rack hover:underline">ProConnect Express Services <ExternalLink className="ml-1 inline-block" size={13} /></a><span className="text-xs">Designed &amp; hosted by <span className="font-semibold text-ink">ProNexa Technology</span></span><Link href="/login" className="font-medium text-rack hover:underline">Client login</Link></div></div></footer>
  </main>;
}
