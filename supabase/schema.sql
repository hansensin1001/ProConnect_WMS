...
create policy warehouses_scoped on warehouses for all
  using (is_org_member(org_id)) with check (is_org_member(org_id));

drop policy if exists zones_scoped on warehouse_zones;
create policy zones_scoped on warehouse_zones for all
  using (is_org_member((select org_id from warehouses w where w.id = warehouse_id)))
  with check (is_org_member((select org_id from warehouses w where w.id = warehouse_id)));

drop policy if exists locations_scoped on locations;
create policy locations_scoped on locations for all
  using (is_org_member((select org_id from warehouses w join warehouse_zones z on z.warehouse_id = w.id where z.id = zone_id)))
  with check (is_org_member((select org_id from warehouses w join warehouse_zones z on z.warehouse_id = w.id where z.id = zone_id)));

drop policy if exists products_scoped on products;
create policy products_scoped on products for all
  using (is_org_member(org_id)) with check (is_org_member(org_id));

drop policy if exists inventory_scoped on inventory_balances;
create policy inventory_scoped on inventory_balances for all
  using (is_org_member((select p.org_id from products p where p.id = product_id)))
  with check (is_org_member((select p.org_id from products p where p.id = product_id)));
...
create policy so_scoped on sales_orders for all
  using (is_org_member(org_id)) with check (is_org_member(org_id));
