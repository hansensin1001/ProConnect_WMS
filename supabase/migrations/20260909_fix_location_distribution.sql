-- Repair for deployments that executed the earlier distribution function.
-- It changes only a read/reporting function; inventory data is untouched.
...
language sql stable security invoker set search_path=public as $$
  with source_stock as (
    ...
    where oi.product_id=p_product_id and so.org_id=p_org_id and so.status='ALLOCATED' and a.picking_location_id is not null group by b.location_id,a.picking_location_id
  ), picked as (select picking_location_id,sum(reserved)::bigint as reserved from picking_reservations group by picking_location_id),
  picked_from_source as (select source_location_id,sum(reserved)::bigint as reserved from picking_reservations group by source_location_id),
  inbound as (
    select poi.location_id,sum(greatest(0,poi.quantity_expected-poi.quantity_received))::bigint as inbound from public.purchase_order_items poi join public.purchase_orders po on po.id=poi.purchase_order_id
    where poi.product_id=p_product_id and po.org_id=p_org_id and po.status in ('DRAFT','PENDING','PARTIALLY_RECEIVED') group by poi.location_id
  ), rows as (
    select s.location_id,s.on_hand,(s.total_reserved-coalesce(ps.reserved,0))::bigint as reserved,(s.on_hand-s.total_reserved)::bigint as available,0::bigint as inbound from source_stock s left join picked_from_source ps on ps.source_location_id=s.location_id
