...
export function isUsername(value: unknown): value is string { return typeof value === "string" && /^[a-z0-9][a-z0-9._-]{2,49}$/i.test(value); }
...
// Address notes can legitimately contain line breaks. Keep all other control
// characters out while allowing conventional CR/LF and tab formatting.
export function isSafeMultilineText(value: unknown, maxLength: number, required = false): value is string {
  return typeof value === "string" && value.length <= maxLength && (!required || value.trim().length > 0) && !/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(value);
}
