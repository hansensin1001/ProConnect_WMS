...
type Location = { id: string; location_code: string };
type Mode = "view" | "edit" | "adjust" | null;

export function InventoryTable({ orgId, rows, locations, canManage }: { orgId: string; rows: Row[]; locations: Location[]; canManage: boolean }) {
  ...
  async function saveProduct(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const patch = { sku: String(form.get("sku") ?? "").trim(), barcode: String(form.get("barcode") ?? "").trim(), name: String(form.get("name") ?? "").trim(), unit_of_measure: String(form.get("uom") ?? "PCS").trim() || "PCS" };
      const { error } = await (supabase.from("products") as any).update(patch).eq("id", selected.id);
  ...
  async function removeProduct(row: Row) {
    try {
      const { error } = await (supabase.from("products") as any).delete().eq("id", row.id);
      if (error) throw error;
  ...
  return <>
    ...
    <Drawer open={mode !== null} onClose={close} title={mode === "view" ? "SKU details" : mode === "edit" ? "Edit SKU" : "Adjust stock"} description={selected ? `${selected.sku} · ${selected.name}` : undefined}>
      ...
      {selected && mode === "edit" && <form onSubmit={saveProduct} className="space-y-4"><label className="block text-sm">SKU code<input name="sku" required defaultValue={selected.sku} className="input-field mt-1 code-label"/></label><label className="block text-sm">Product name<input name="name" required defaultValue={selected.name} className="input-field mt-1"/></label><label className="block text-sm">Barcode<input name="barcode" required defaultValue={selected.barcode} className="input-field mt-1 code-label"/></label><label className="block text-sm">Unit of measure<input name="uom" required defaultValue={selected.unit_of_measure} className="input-field mt-1"/></label>{message && <p role="alert" className="text-sm text-alert">{message}</p>}<div className="flex justify-end gap-2 border-t border-line pt-5"><button type="button" onClick={close} className="btn-secondary">Close</button><button className="btn-primary">Save changes</button></div></form>}
      {selected && mode === "adjust" && <form onSubmit={adjustStock} className="space-y-4"><p className="rounded-md border border-amber/30 bg-amber/10 p-3 text-sm text-graphite">Adjustments are audited. Use a positive number to add stock and a negative number to remove it.</p><label className="block text-sm">Bin location<select name="locationId" required className="input-field mt-1"><option value="">Select bin…</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.location_code}</option>)}</select></label><label className="block text-sm">Quantity change<input name="quantity" type="number" step="1" required className="input-field mt-1"/></label><label className="block text-sm">Reason<textarea name="reason" required className="input-field mt-1 min-h-20"/></label>{message && <p role="alert" className="text-sm text-alert">{message}</p>}<div className="flex justify-end gap-2 border-t border-line pt-5"><button type="button" onClick={close} className="btn-secondary">Cancel</button><button className="btn-primary">Record adjustment</button></div></form>}
    </Drawer>
  </>;
}
