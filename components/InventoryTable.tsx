...
type Row = { id: string; sku: string; barcode: string; name: string; description?: string | null; price?: number; unit_of_measure: string; onHand: number; reserved: number; locations: number };
...
export function InventoryTable({ orgId, rows, canManage, page, total }: { orgId: string; rows: Row[]; canManage: boolean; page: number; total: number }) {
  ...
  async function saveProduct(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const patch = { barcode: String(form.get("barcode") ?? "").trim(), name: String(form.get("name") ?? "").trim(), description: String(form.get("description") ?? "").trim() || null, price, unit_of_measure: String(form.get("uom") ?? "PCS").trim() || "PCS" };
