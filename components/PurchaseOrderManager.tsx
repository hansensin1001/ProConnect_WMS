...
import { binLabel, warehouseError, appendSerials } from "@/lib/warehouse-ui";
...
type ReceiptLine = { id: string; product_id: string; location_id: string; quantity_expected: number; quantity_received: number; products?: Product; locations?: Location };
...
export function PurchaseOrderManager({ orgId, canManage, initialOrders, products, locations, page, total }: { orgId: string; canManage: boolean; initialOrders: Order[]; products: Product[]; locations: Location[]; page: number; total: number }) {
  ...
  useEffect(() => { if (receiving) Object.values(scanInputs.current)[0]?.focus(); }, [receiving]);
  ...
  const isSerializedLine = (line: ReceiptLine) => Boolean((line.products ?? productFor(line.product_id))?.is_serialized);

  function beginCreate() {
    setEditing(null); setSupplier(""); setLines([{ productId: "", locationId: locations[0]?.id ?? "", quantity: 1 }]); setMsg(""); setOpen(true);
  }
  ...
  function addSerials(line: ReceiptLine, values: string[]) {
    ...
    for (const serial of values) {
      if (existingElsewhere.has(serial) || next.includes(serial)) { setMsg(`Serial ${serial} was already added to this receipt. Batch not added.`); return; }
      if (next.length >= line.quantity_expected) { setMsg(`This line requires exactly ${line.quantity_expected} serial numbers. Batch not added.`); return; }
      next.push(serial);
    }
  ...
  function commitScan(line: ReceiptLine) {
    ...
    scanInputs.current[line.id]?.focus();
  }
  ...
  function receiptReady() {
    if (!receiving || !receiving.purchase_order_items?.length || importingLine !== null) return false;
    return (receiving.purchase_order_items ?? []).every((line) => Boolean(line.products ?? productFor(line.product_id)) && Boolean(line.location_id) && (!isSerializedLine(line) || (receiptSerials[line.id] ?? []).length === line.quantity_expected));
  }

  async function save(event: FormEvent) {
    event.preventDefault();
    if (saveLock.current) return;
    const valid = lines.filter((line) => line.productId && line.locationId && Number.isInteger(line.quantity) && line.quantity > 0);
    if (!supplier.trim() || !valid.length || valid.length !== lines.length) return setMsg("Add a supplier, SKU, receiving bin, and quantity.");
    ...
    try {
      ...
      setOpen(false);
    } catch (error) { setMsg(warehouseError(error, "Unable to save purchase order.")); }
  ...
  async function receiveStock() {
    if (!receiving || !receiptReady() || receiveLock.current) return;
    ...
    finally { receiveLock.current = false; setReceivingBusy(false); }
  }
  ...
  return <div className="space-y-4">
    ...
    <WorkflowNotice notice={msg ? { kind: "error", text: msg } : null} />
    <div className="overflow-hidden rounded-lg border border-line bg-panel"><div className="overflow-x-auto"><table className="mobile-cards w-full min-w-[700px] text-sm"><thead className="bg-paper text-left text-[11px] uppercase tracking-wide text-graphite"><tr><th className="px-5 py-3">PO reference</th><th className="px-5 py-3">Supplier</th><th className="px-5 py-3">Lines</th><th className="px-5 py-3">Status</th><th className="px-5 py-3" /></tr></thead><tbody>{orders.map((order) => { const received = ["RECEIVED", "COMPLETED"].includes(order.status); return <tr key={order.id} className="border-t border-line hover:bg-paper/70"><td data-label="PO reference" className="px-5 py-3"><button onClick={() => beginView(order)} className="code-label font-medium text-rack hover:underline">{order.po_number}</button></td><td data-label="Supplier" className="px-5 py-3">{order.supplier_name ?? "—"}</td><td data-label="Lines" className="px-5 py-3">{order.purchase_order_items?.length ?? 0}</td><td data-label="Status" className="px-5 py-3"><StatusBadge status={order.status} /></td><td data-label="Actions" className="px-5 py-3"><RowActions onView={() => beginView(order)} onEdit={canManage && ["DRAFT", "PENDING"].includes(order.status) ? () => beginEdit(order) : undefined} extraActions={canManage ? [...(order.status === "PENDING" ? [{ label: "Process receipt", onClick: () => beginReceive(order) }] : []), ...((order.status === "PENDING" || received) ? [{ label: order.status === "PENDING" ? "Cancel PO" : "Rollback receipt", onClick: () => action(order, "rollback"), destructive: true }] : [])] : []} /></td></tr>; })}</tbody></table></div><PageNavigation page={page} pageSize={50} total={total} /></div>

    <Drawer open={open} onClose={() => !saving && setOpen(false)} title={editing ? "Edit purchase order" : "Create purchase order"} description="Inbound stock changes only when the PO is received."><form onSubmit={save} className="space-y-5"><WorkflowNotice notice={msg ? { kind: "error", text: msg } : null} /><label className="block text-sm">Supplier<input value={supplier} onChange={(event) => setSupplier(event.target.value)} required className="input-field mt-1" /></label><div><p className="text-xs font-semibold uppercase tracking-wide text-graphite">Expected receipts</p><div className="mt-3 space-y-3">{lines.map((line, index) => <div key={index} className="grid gap-2 md:grid-cols-[1fr_1fr_90px]"><select value={line.productId} onChange={(event) => updateLine(index, { productId: event.target.value })} className="input-field"><option value="">Master SKU…</option>{products.map((product) => <option key={product.id} value={product.id}>{product.sku} — {product.name}{product.is_serialized ? " · Serialized" : ""}</option>)}</select><select value={line.locationId} onChange={(event) => updateLine(index, { locationId: event.target.value })} className="input-field"><option value="">Receiving bin…</option>{locations.map((location) => <option key={location.id} value={location.id}>{binLabel(location)}</option>)}</select><input type="number" min="1" value={line.quantity} onChange={(event) => updateLine(index, { quantity: Number(event.target.value) })} className="input-field" /></div>)}</div><button type="button" onClick={() => setLines((current) => [...current, { productId: "", locationId: current[current.length - 1]?.locationId || locations[0]?.id || "", quantity: 1 }])} className="btn-secondary mt-3">Add line</button></div><div className="flex justify-end gap-2 border-t border-line pt-5"><button type="button" onClick={() => setOpen(false)} className="btn-secondary">Cancel</button><button disabled={saving} className="btn-primary"><Truck size={16} />{saving ? "Saving…" : editing ? "Save PO" : "Create PO"}</button></div></form></Drawer>
    ...
    <Drawer open={receiving !== null} onClose={() => !receivingBusy && setReceiving(null)} title="Process purchase receipt" description={receiving ? `${receiving.po_number} · serialised lines must match the received quantity exactly` : undefined}>{receiving && <div className="space-y-5"><section className="rounded-md border border-line bg-paper p-3 text-sm"><p className="font-medium">{receiving.supplier_name ?? "Supplier"}</p><p className="mt-1 text-xs text-graphite">Scan each serial into its matching SKU line. CSV, TXT, and standard .xlsx files import the first column.</p></section>{(receiving.purchase_order_items ?? []).map((line) => { const product = line.products ?? productFor(line.product_id); const location = line.locations ?? locationsById.get(line.location_id); const serials = receiptSerials[line.id] ?? []; if (!product?.is_serialized) return <div key={line.id} className="rounded-md border border-line p-4 text-sm"><div className="flex justify-between gap-3"><span><span className="code-label">{product?.sku ?? "SKU"}</span><span className="ml-2">{product?.name ?? "Product unavailable"}</span></span><span>{line.quantity_expected} units → {location?.display_code ?? location?.location_code ?? "Bin"}</span></div><p className="mt-2 text-xs text-graphite">Standard SKU — no serial registration required.</p></div>; return <section key={line.id} className="rounded-lg border border-violet-200 bg-violet-50/40 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-medium"><span className="code-label">{product.sku}</span><span className="ml-2">{product.name}</span></p><p className="mt-1 text-xs text-graphite">Receive {line.quantity_expected} into {location?.display_code ?? location?.location_code ?? "selected bin"}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${serials.length === line.quantity_expected ? "bg-emerald-100 text-go" : "bg-amber/20 text-amber-dark"}`}>{serials.length} / {line.quantity_expected} scanned</span></div><div className="mt-4 flex gap-2"><div className="relative flex-1"><ScanLine size={16} className="absolute left-3 top-3 text-graphite" /><input ref={(node) => { scanInputs.current[line.id] = node; }} disabled={receivingBusy} autoComplete="off" spellCheck={false} value={scanValues[line.id] ?? ""} onChange={(event) => handleScanChange(line, event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" || (event.key === "Tab" && (scanValues[line.id] ?? "").trim())) { event.preventDefault(); commitScan(line); } }} placeholder="Scan serial, then press Enter" className="input-field pl-9" /></div><button type="button" onClick={() => commitScan(line)} className="btn-secondary">Add</button><label className="btn-secondary cursor-pointer"><FileUp size={16} />{importingLine === line.id ? "Importing…" : "Import"}<input type="file" accept=".csv,.txt,.xlsx" disabled={importingLine !== null} onChange={(event) => void importSerialFile(line, event.target.files?.[0] ?? null)} className="sr-only" /></label></div>{serials.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{serials.map((serial) => <span key={serial} className="inline-flex items-center gap-1 rounded bg-panel px-2 py-1 text-xs code-label"><span>{serial}</span><button type="button" onClick={() => removeSerial(line.id, serial)} aria-label={`Remove ${serial}`} className="text-alert"><Trash2 size={13} /></button></span>)}</div>}</section>; })}<WorkflowNotice notice={msg ? { kind: "error", text: msg } : null} /><div className="flex justify-end gap-2 border-t border-line pt-5"><button type="button" disabled={receivingBusy} onClick={() => setReceiving(null)} className="btn-secondary">Cancel</button><button type="button" disabled={!receiptReady() || receivingBusy} onClick={() => void receiveStock()} className="btn-primary"><Truck size={16} />{receivingBusy ? "Receiving…" : "Receive stock"}</button></div></div>}</Drawer>
  </div>;
}
