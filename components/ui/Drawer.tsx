...
export function Drawer({ open, onClose, title, description, children }: { open: boolean; onClose: () => void; title: string; description?: string; children: ReactNode }) {
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-50 flex justify-end bg-ink/35 backdrop-blur-[1px]" role="dialog" aria-modal="true" aria-label={title}>
    <button className="flex-1 cursor-default" onClick={onClose} aria-label="Close panel" />
    <aside className="h-full w-full max-w-xl overflow-y-auto bg-panel shadow-2xl animate-[slide-in_.18s_ease-out]">
      <div className="sticky top-0 z-10 flex items-start justify-between border-b border-line bg-panel px-6 py-5">
