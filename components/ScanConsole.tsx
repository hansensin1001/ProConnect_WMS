...
export function ScanConsole({
  ...
  const [location, setLocation] = useState<{ id: string; display_code?: string; location_code: string } | null>(null);
  ...
  const lookupLocation = useCallback(
    async (code: string) => {
      try {
        const [physicalResult, displayResult] = await Promise.all([
          supabase.from("locations").select("id, display_code, location_code, warehouse_zones!inner(warehouse_id)").eq("location_code", code).eq("warehouse_zones.warehouse_id", warehouseId).maybeSingle(),
          supabase.from("locations").select("id, display_code, location_code, warehouse_zones!inner(warehouse_id)").eq("display_code", code).eq("warehouse_zones.warehouse_id", warehouseId).maybeSingle(),
        ]);
        const data = physicalResult.data ?? displayResult.data;
        if (physicalResult.error || displayResult.error || !data) throw new Error(`No location found for "${code}" in this warehouse.`);
        setLocation(data);
        setStep("quantity");
        setScanning(false);
        setStatus(null);
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setStatus({ type: "ok", message: `${mode === "PUTAWAY" ? "Put away" : "Picked"} ${quantity} × ${product.sku} at ${location.display_code ?? location.location_code}.` });
      setStep("done");
      ...
      <div className="bg-panel border border-line p-6">
        ...
        {location && (
          <div className="mb-4 text-sm bg-paper border border-line px-3 py-2">
            ...
            <span className="code-label">{location.display_code ?? "LOC?"}</span><span className="text-graphite"> · {location.location_code}</span>
          </div>
        )}
