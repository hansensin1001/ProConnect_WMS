...
type CarrierCode = "DHL_EXPRESS" | "NINJA_VAN";
type Carrier = {
  id: string;
...
const carrierNames: Record<CarrierCode, string> = { DHL_EXPRESS: "DHL Express", NINJA_VAN: "Ninja Van" };
const emptyForm = (): FormState => ({ carrierCode: "DHL_EXPRESS", displayName: "DHL Express", isActive: false, isSandbox: true, apiKey: "", apiSecret: "", accountNumber: "", clientId: "", webhookSecret: "" });
...
export function CarrierManager({ orgId, orgName }: { orgId: string; orgName: string }) {
  ...
  function openCreate() {
    ...
    if (!code) return setMessage("DHL Express and Ninja Van are already configured. Edit an existing carrier to change its settings.");
