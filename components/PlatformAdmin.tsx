...
type Org = { id: string; name: string; slug: string; deployment_mode: string; org_members: { id: string; user_id: string; role: string }[] };
...
export function PlatformAdmin() {
  ...
  return <div className="max-w-4xl space-y-6">
    ...
    <section className="border border-line bg-panel"><div className="border-b border-line px-5 py-3 text-sm font-medium">Organizations and assigned users</div>{orgs.map((org) => <div key={org.id} className="border-b border-line px-5 py-4 last:border-0"><div className="font-medium">{org.name} <span className="code-label text-xs text-graphite">{org.slug}</span></div><div className="mt-2 space-y-1 text-sm text-graphite">{org.org_members.length ? org.org_members.map((member) => <div key={member.id}><span className="code-label text-xs">{member.user_id}</span> — {member.role}</div>) : "No users assigned."}</div></div>)}</section>
  </div>;
}
